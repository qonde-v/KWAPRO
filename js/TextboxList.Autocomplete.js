...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
	...
	var search = function(bit){
		...
		if (options.queryRemote){
			if (searchValues[search]){
				...
				var data = (typeof(options.remote.extraParams) == 'undefined') ? {} : options.remote.extraParams;
	...
	var showResults = function(search){
		var results = values; //method.filter(values, search, options.insensitive, options.maxResults);
	...
	var focus = function(element){
		...
		if(element.attr('title') && element.is(":visible"))
		{
			$('.textboxlist-autocomplete').prepend("<img style='float:right;z-index:9' id='f_thumb' width='37' height='39' src='"+element.attr('title')+"' />");
		}
	};
