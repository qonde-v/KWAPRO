  ...
  this.is = function(t){
	...
	if (type == 'editable'){
		...
		var toBox = function(){
			...
			{
				var b = new Array();
				if(escape(value[1]).indexOf("%u")!=-1)
				{
					var str_arr = value[1].split(" ");
					for(var i=0;i<str_arr.length;i++)
					{
						b[i] = textboxlist.create('box', [null,str_arr[i],null]);
					}
				}
				else
				{
					b[0] = textboxlist.create('box', [null,value[1],null]);
				}
			}
			if (b){
				for(var i=0;i<b.length;i++)
				{
					b[i].inject(bit, 'before');
				}
