   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'number', validate: function(value) {return (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(value));}, defaultMsg: '请输入数字。'},
             {name: 'ismobile', validate: function(value) {return ismobile($.trim(value));}, defaultMsg: '请输入正确的联系方式。'},
              {name: 'iscard', validate: function(value) {return isIdCardNo($.trim(value));}, defaultMsg: '请输入正确的身份证号码。'},
...
{ 
    ...
    var validateField = function(field, valid) { 
        ...
        var someinput=(el.attr('someinput')?el.attr('someinput'):null);
        ...
        if ( minlength && !error){
            ...
            if (error && (msg==null || errorMsg=='')){
                errorMsg = '必须大于等于' + minlength +'位';
            }       
        };
        //验证是否相同
        if ( someinput && !error){
            error = el.val() != $("#"+someinput).val();
            if (error && (msg==null || errorMsg=='')){
                errorMsg = '两次输入密码必须一致';
            }else{
             var controlGroup = $("#"+someinput).parents('.form-group');
			 if($("#"+someinput).parents('.form-group').find("#valierr").html()=='两次输入密码必须一致'){
